...
class ExistingAccountLoginTests(unittest.TestCase):
    ...
    def test_refresh_token_stage_rotates_mfa_with_fresh_web_session_before_oauth(self):
        ...
        with mock.patch(
            "platforms.chatgpt.refresh_token_registration_engine.ChatGPTMfaManager.logout_all_devices"
        ):
            result = engine.run()
        ...
        oauth_client.adopt_browser_context.assert_not_called()
        oauth_kwargs = oauth_client.login_and_get_tokens.call_args.kwargs
        self.assertTrue(oauth_kwargs["force_new_browser"])
        self.assertFalse(oauth_kwargs["resume_authenticated_session"])
    ...
    def test_post_logout_oauth_failure_does_not_reuse_revoked_session(self):
        engine = self._make_engine(
            email_service=PasswordTotpEmailService(),
            rotate_mfa=True,
        )
        ...
        oauth_client.login_and_get_tokens.assert_called_once()
        oauth_client.adopt_browser_context.assert_not_called()
        kwargs = oauth_client.login_and_get_tokens.call_args.kwargs
    ...
    def test_web_login_resolves_workspace_page_before_generic_navigation(self):
        client = ChatGPTClient(verbose=False)
        client.visit_homepage = mock.Mock(return_value=True)
        client.get_csrf_token = mock.Mock(return_value="csrf-token")
        ...
        client._follow_flow_state = mock.Mock(return_value=(True, callback_state))
        ...
        with mock.patch.object(
            ...
            return_value=(None, callback_state),
        ) as submit_workspace:
            ok, result = client.login_existing_account_and_get_session(
                "existing@example.com",
                mock.Mock(),
